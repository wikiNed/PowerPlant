...
var bLayerShow = false; //图层是否显示

/*---------------------------------------工具栏START---------------------------------*/
//图层管理
function LayerManagerClicked(id) {
	var flag = EarthMenu.isMenuClicked(id);	// 判断菜单之前的状态，如果已经打开，则关闭，反之打开
	setLayerShow(flag);
	bLayerVisible = flag;
}
...
function layerTransClicked(id) {
	var flag = EarthMenu.isMenuClicked(id);
	if(flag) { //显示
...
function QueryPropertyClicked() {
	...
	//PowerPlant.propertyQuery();
}

function powerPlantClicked() {
	LayerManagement.earth.focus();
...
function cameraQueryClicked() {
	if( PowerPlant.cameraOutClick ){
		...
		PowerPlant.cameraOutClick = false;
		PowerPlant.setPowerPlantQuery();
	}else{
		for (var key in PowerPlant.iconObjects) {
			if( PowerPlant.iconObjects[key].type == 'cameraout' || PowerPlant.iconObjects[key].type == 'camerain'){
		...
		PowerPlant.cameraOutClick = true;
		PowerPlant.closePowerPlantQuery();
	}
}

function deviceQueryClicked() {
	PowerPlant.showDeviceTree();
}

function ndeviceQueryClicked() {
	// PowerPlant.outputModel();//输出模型数据
	PowerPlant.showPropertyTreeBallon(PowerPlant.deviceTreeNodes);
}

function allCameraTreeClicked() {
...
//漫游
function wanderClicked() {
	if (!PowerPlant.wanderClicked){
		PowerPlant.getDynamic('消防员');
		setTimeout(function () {
			PowerPlant.enterTrack(PowerPlant.dynamic.Guid);
		},500);
		PowerPlant.wanderClicked = !PowerPlant.wanderClicked;
	}else{
		PowerPlant.outTracking(PowerPlant.dynamic.Guid);
		PowerPlant.wanderClicked = !PowerPlant.wanderClicked;
	}
...
//一键巡查
function partolClicked() {
	PowerPlant.patrolAll();
}

function pandectClicked() {
...
function historyDataClicked(id) {
	...
	isCloseSlider = true;	// 去掉top
	var flag = EarthMenu.isMenuClicked("ViewTranSetting");
	if(flag) {
		//BalloonHtml.removeItemStle("ViewTranSetting");	//EarthMenu Modify	//去掉top
		EarthMenu.cancelMenuClick("ViewTranSetting");
		setSlidersVisible(0);	//去掉top
		earth.Event.OnGUISliderChanged = function() {};
	}
	flag = EarthMenu.isMenuClicked(id);
	if (flag) {
...
function historyCompareClicked(id) {
	bMultiple = false; // 当前是否是多屏显示状态	//去掉top
...
function SpecialEffectClicked(id) {
	showDialog("html/menu/three_menu_item.html?menuType=1", id, function() {
		...
		if (EarthMenu.isMenuClicked("ViewTranSetting")) {
			return;
		} else {
			setSlidersVisible(0);
		}
...
function ViewHawkEyeClicked() {
	var flag = Tools.toolBarItemClickStyle("ViewHawkEye");	//todo
...
function keywordSearchClicked(id) {
	var selectLayer = LayerManagement.selectLayer;	//去掉top
	if(!selectLayer) {
		alert("请选择一个图层");
		return;
	} else {

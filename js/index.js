...
var isCloseSlider = false;	//是否有滑动窗口需要关闭
...
var headerHeight = 0; //标题栏高度
//var dialogLeft = Math.ceil(86 * zoomInit); //弹出框靠左距离
var dialogLeft = 86 + 135; //弹出框靠左距离
...
function init() {
	...
	showMenu("hori","0");
	//生成设备树
	PowerPlant.showAllDeviceTree(deviceList);
	//生成摄像设备树
	PowerPlant.showAllCameraTree(allCameraList);
	//视角
	earth.GlobeObserver.FlytoLookat(109.413576, 38.034936, 1058.73,
		...
		5);  		//时间
// 	视角
// 	earth.GlobeObserver.FlytoLookat(104.763029, 31.398541, 453.73,
// 		185.442,  //朝向
// 		-29.228,  //俯仰
// 		220,      //旋转角
// 		450,		//相机距离
 ...
 */
/*var earthToolHeightTemp = 0; //工具栏临时高度	// EarthMenu Modify
 ...
 */
/*function resizeEarthTool(ballonHeightNow) {	// EarthMenu Modify
...
function setBalloonVisible(isVisible){
	if(LayerManagement.htmlBalloon) {	//去掉top
		LayerManagement.htmlBalloon.SetIsVisible(isVisible);	//去掉top
	}
	/*if(earthToolsBalloon) {	// EarthMenu Modify
	...
	if (EarthMenu != undefined && EarthMenu != null) {
		EarthMenu.setMenuVisible(isVisible);
	}
	// if (CampAPI != undefined && CampAPI != null) {
	// 	CampAPI.setBalloonVisible(isVisible);
	// }
}
/**
...
function setLayerShow(show) {
	...
	if(show) {
		$("#leftPanel").show();
		$(".layerHeaderInnerText").html("图层管理")
		$("#id_tree_body").show();
		...
		if(dialogId) {
			//BalloonHtml.setItemStyle("LayerManager");	//EarthMenu Modify
			bLayerVisible = true;
			closeDialog();
			return;
		}
	...
	initSystemParam: function(earth) {
		...
		if(SYSTEMPARAMS.project != "") {
			if(SYSTEMPARAMS.Position != "" && SYSTEMPARAMS.Position) {
				...
				earth.GlobeObserver.GotoLookat( longitude, latitude, altitude, tilt, heading, roll, range);
			}
...
var CoordinateTransform = {
	...
	createDatum: function(spatialUrl) {
		...
		var dataProcess = getDataProcessIndex();	//去掉top
		dataProcess.Load();
...
function pictureHtml(tag, sAltitudeType) {
	...
	picturesBalloons.SetScreenLocation(width / 2 + dialogLeft, 0);	//去掉top
...
var clearSearchResult = function(currentLayer) {
	if(globalSearchLayer != null) {	//去掉top
		globalSearchLayer.ClearSearchResult();	//去掉top
	}
	globalSearchLayer = currentLayer;	//去掉top
}

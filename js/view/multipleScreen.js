...
$(function() {
	var bMultiple = parent.bMultiple; // 当前是否是多屏显示状态	//去掉top
	var earth = parent.LayerManagement.earth; //接收全局earth	//去掉top
	...
	var getLayerData = function(layer, bWithIcon) {
		...
		for(var i = 0; i < childCount; i++) {
			...
			if(bWithIcon) {
				data["icon"] = parent.LayerManagement.getLayerIcon(layerType, true);	//去掉top
			}
	...
	$("#rdoLeft").click(function() {
		...
		earthTag = 0;
		createLayerTree("#layerTree", parent.LayerManagement.earthArray[0]);	//去掉top
	});
	$("#rdoRight").click(function() {
		$("#rdoLeft").removeAttr("checked");
		$("#rdoRight").attr("checked", "checked");
		earthTag = 1;
		createLayerTree("#layerTree", parent.LayerManagement.earthArray[1]);	//去掉top
	});
	$("#btnCompare").click(function() {
		if(bSync || parent.bSync) {	//去掉top
			bSync = false; //首先关闭联动
			parent.bSync = bSync;	//去掉top
			parent.setSync(bSync);	//去掉top
			setAlphaSync(false);
		}
		if(bMultiple) { //当前多屏时
			if(type == 2) {
				parent.LayerManagement.showHistorySlider(false, true, true);	//去掉top
			}
			$(this).text("双屏显示");
			parent.isShowHistory = false;	//去掉top
			parent.setScreen(1, ""); // 恢复到一屏	//去掉top
			bMultiple = false;
			parent.bMultiple = bMultiple;	//去掉top
			...
			createLayerTree("#layerTree", parent.LayerManagement.earthArray[0]); // 恢复为默认球的图层树	//去掉top
			if(type == 2) {
				if(!($(parent.document).find("#historyData").hasClass("selected"))) {
					parent.LayerManagement.showHistorySlider(false);	//去掉top
				}
			}
		} else { //当前单屏时
			debugger
			parent.isCloseSlider = true;	//去掉top
			$(this).text("单屏显示");
			bMultiple = true;
			parent.bMultiple = bMultiple;	//去掉top
			...
			parent.setScreen(2, selectTag, false, function() {	//去掉top
				debugger
				if(type == 2) {
					parent.isShowHistory = true;	//去掉top
					var flag = parent.EarthMenu.isMenuClicked("ViewTranSetting");	//去掉top
					if(!flag) {
						//parent.BalloonHtml.removeItemStle("ViewTranSetting");	//EarthMenu Modify	//去掉top
						parent.setSlidersVisible(0);	//去掉top
						earth.Event.OnGUISliderChanged = function() {};
						setTimeout(function() {
							// var flag2 = parent.Tools.toolBarItemClickStyle("historyData");	//去掉top
							parent.LayerManagement.showHistorySlider(true);	//去掉top
						}, 300);
					} else {
						// var flag2 = parent.Tools.toolBarItemClickStyle("historyData");	//去掉top
						parent.LayerManagement.showHistorySlider(true);	//去掉top
				...
				parent.gotoPose(0)(); // 将其他屏定位到第一屏的位置	//去掉top
			});
...
function setAlphaSync(isMulti) {
	parent.isMultiScreen_2 = isMulti;	//去掉top
	if(isMulti) {
		if(parent.LayerManagement.earthArray[1]) {	//去掉top
			parent.LayerManagement.earthArray[1].Environment.TerrainTransparency = parent.LayerManagement.earthArray[0].Environment.TerrainTransparency;	//去掉top
		}
	}
}
...
window.onbeforeunload = function() {
	...
	parent.isShowHistory = false;	//去掉top
	parent.setScreen(1, "");	//去掉top
	bSync = false;
	parent.bSync = false;	//去掉top
	parent.setSync(bSync);	//去掉top
	setAlphaSync(false);	//去掉top
	if(type == 2) {
		if(!($(parent.document).find("#historyData").hasClass("selected"))) {
			parent.LayerManagement.showHistorySlider(false);	//去掉top
		}

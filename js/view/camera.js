 ...
 ******************************************/
$(function() {
	parent.cameras = []; //摄像机数组	//去掉top
	var earth = parent.earth; //全局earth
	var divHeight = $(parent.document).height() - 380;
	parent.cameraLayer = earth.Factory.CreateEditLayer(earth.Factory.CreateGuid(), "my_layer", earth.Factory.CreateLonLatRect(-90, 90, -180, 180, 0, 10), 0, 10, '');	//去掉top
	earth.AttachObject(parent.cameraLayer);	//去掉top
	...
	$("#btnNew").click(function() {
		earth.Event.OnCreateGeometry = function(pt, type) {
			if(pt) {
				...
				parent.cameraLayer.BeginUpdate();		//去掉top
				parent.cameraLayer.AttachObject(camera);//去掉top
				parent.cameraLayer.EndUpdate();			//去掉top

				parent.cameras[guid] = camera;			//去掉top
				...
				if(camerasTrees.getNodes()[0].children.length < 2) {
					//parent.resizeEarthTool(parent.earthToolHeight * 3 / 4);	// EarthMenu Modify
					parent.EarthMenu.resizeMenuHeight(Math.floor(parent.EarthMenu.getMenuHeight() * 3 / 4));	//去掉top
				}
	...
	function createCamera(record) {
		...
		parent.cameras[record["ID"]] = camera;	//去掉top
	}
			...
			if(confirm("是否确定要删除？")) {
				...
				for(var i = 0; i < camera.length; i++) {
					...
					if(id === userId) {
						...
						if(parent.cameras.hasOwnProperty(id)) {	//去掉top
							earth.ToolManager.SphericalObjectEditTool.Browse();
							var thisCamera = parent.cameras[id];	//去掉top
							if(thisCamera) {
								if(thisCamera.EnableMediaStream) {
									thisCamera.EnableMediaStream = false;
								}
							}
							parent.cameraLayer.DetachWithDeleteObject(thisCamera);	//去掉top
							parent.cameras[id] = "";	//去掉top
						}
						//保存文档
				...
				if(rootNode[0].children.length < 1) {
					//parent.resizeEarthTool(); //还原工具条高度
		...
		if(userId) {
			...
			for(var i = 0; i < camera.length; i++) {
				...
				if(id === userId) {
					...
					var camera = parent.cameras[userId];	//去掉top
					camera.RtspURL = obj.rtsp;
					camera.MaskURL = obj.maskUrl;
			...
			callback: {
				...
				onClick: function(event, treeId, node) {
					if(node && node.id) {
						...
						var camera = parent.cameras[node.id];	//去掉top
						if(camera) {
							//控制摄像机窗口最前
							closeCameraWindows();
	...
	function exportCameras(obj) {
		...
		for(var i = 1; i < nodes.length; i++) {
			var camera = parent.cameras[nodes[i].id];	//去掉top
	...
	function getSelectedCamera() {
		...
		var camera = null;
		if(selNode) {
			camera = parent.cameras[selNode.id];	//去掉top
		}
		return camera;
	}
	...
	function closeCameraWindows() {
		for(var i in parent.cameras) {	//去掉top
			if(parent.cameras[i]) {		//去掉top
				parent.cameras[i].EnableThumbWindow = false;	//去掉top
			}
		}
	}
	/**
	...
	function close() {
		if(earth.UserDocument) {
			for(var id in parent.cameras) {	//去掉top
				if(parent.cameras.hasOwnProperty(id)) {	//去掉top
					if(parent.cameras[id] != "") {	//去掉top
						parent.cameraLayer.DetachWithDeleteObject(parent.cameras[id]);	//去掉top
					}
				}
			}
			earth.DetachObject(parent.cameraLayer);	//去掉top
			parent.cameras = [];		//去掉top
			parent.cameraLayer = null;	//去掉top
		}
	}

...
$(function() {
	...
	$("#btnConfirm").click(function() {
		...
		if(cameraRotationAngle >= parent.params.camearaMaxRotation && horizonRotate > 0){	//去掉top
		...
		if(currentCameraTilt >= parent.params.camearaMaxTilt && verRotate > 0){	//去掉top
		...
		if(cameraRotationAngle >= parent.params.camearaMaxRotation){//旋转至最右侧(350度)，停止旋转	//去掉top
			horizonRotate = parent.params.camearaMaxRotation - cameraDictionary[curSelNodeId].cameraRotationAngle;	//去掉top
			cameraDictionary[curSelNodeId].cameraRotationAngle = parent.params.camearaMaxRotation;	//去掉top
		...
		if(currentCameraTilt >= parent.params.camearaMaxTilt){//旋转至最上方侧(90度)，停止旋转	//去掉top
			verRotate = parent.params.camearaMaxTilt - cameraDictionary[curSelNodeId].cameraTiltAngle;	//去掉top
			cameraDictionary[curSelNodeId].cameraTiltAngle = parent.params.camearaMaxTilt;	//去掉top
	...
	var events = {
		"keyup": function(event) {
			...
			if(id == "horizonRotate") { //左右旋转
				checkNum(thisObj, false, 2, parent.params.camearaMaxRotation);	//去掉top
			} else if(id == "verRotate"){
				checkNum(thisObj, false, 2, parent.params.camearaMaxTilt);		//去掉top
			}else if(id == "leftRightSpeed") {
				checkNum(thisObj, true, 2, parent.params.camearaMaxRotation);	//去掉top
			}else if(id == "updownSpeed") {
				checkNum(thisObj, true, 2, parent.params.camearaMaxTilt);		//去掉top
			}
...
window.onbeforeunload = function(event) {
	...
	//parent.resizeEarthTool();
	parent.EarthMenu.resizeMenu();		//去掉top
}
...
function initDeviceList(callFuc) {
	...
	gb28181.Gb28181Server = parent.params.ip;	//去掉top
...
function getCameraList(cameraListXml) {
	...
	if(0 < resultnum) {
		...
		for(i = 0; i < resultnum; i++) {
			...
			//调用接口获取数据库相关字段
			earth.Gb28181CameraManager.Gb28181Server = parent.params.ip;	//去掉top
			...
			if(!parent.params.isSimulationCamera) { //不启用模拟摄像头则开启过滤	//去掉top
...
function createCamera(lon, lat, deviceID, num, tilt, heading, rotation) {
	...
	var altitude = earth.Measure.MeasureTerrainAltitude(lon, lat) + parent.params.cameraDefaultHeight; //默认离地高度10米	//去掉top
	elementCone.SphericalTransform.SetLocationEx(lon, lat, altitude);
	elementCone.SphericalTransform.SetRotationEx( 90, parent.params.cameraDefaultHeading, 0 );	//去掉top
	...
	camera.SetGB28181ServerUrl(parent.params.ip);	//去掉top
 ...
 * 调整工具条高度
 */
function resetEarthToolHeight() {	//EarthMenu Modify todo
	var oLength = 0;
	for(var i in cameras) {
		oLength++;
	}
	//if(parent.earthToolsBalloon && oLength > 0) { //工具条调整	//去掉top
	if(oLength > 0) { //工具条调整
		parent.hideProfile();
		if(oLength <= 4) { //减少1/4
			//parent.resizeEarthTool(parent.earthToolHeight * 3 / 4);	//去掉top
			parent.EarthMenu.resizeMenuHeight(parent.EarthMenu.getMenuHeight() * 3 / 4);	//去掉top
		} else { //减少1/2
			//parent.resizeEarthTool(parent.earthToolHeight / 2);	//去掉top
			parent.EarthMenu.resizeMenuHeight(parent.EarthMenu.getMenuHeight() / 2);	//去掉top
		}
	} else { //还原
		//parent.resizeEarthTool();	//去掉top
		parent.EarthMenu.resizeMenu();	//去掉top
	}
}

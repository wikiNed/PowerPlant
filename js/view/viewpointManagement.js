...
window.onload = function() {
	...
	$("#close_iframe").click(function() {
		parent.ViewPointManagementBtn = false;	//去掉top
		parent.setTableView(false, 135);
		//parent.resizeEarthTool();				//去掉top
		parent.EarthMenu.resizeMenu();
		//parent.Tools.toolBarItemClickStyle("ViewPointManagement");	//去掉top
		parent.EarthMenu.cancelMenuClick("ViewPointManagement");		//去掉top
	});
	//上一页
...
var getWebView = function() {
	var path = parent.params.ip + "/viewImages/";	//去掉top
	return path;
}
/**
 * 添加缩略图
...
var getViewImageDir = function() {
	var dir = parent.params.ip.replace(/^http:\/{2}/g, '\\\\');	//去掉top

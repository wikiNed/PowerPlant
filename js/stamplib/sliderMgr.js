...
(function(ns) {
	...
	var _sliders = [{
		...
		title: '地形透明',
		slider: null,
		percent: parseInt(params.transparency) >= 0 ? parseInt(params.transparency) : 100,	//去掉top
		...
		setVisible: function(type, visible) {
			...
			if(!slider.slider) {
				...
				if(slider.type == "transparency") {
					if(parseInt(params.transparency) || parseInt(params.transparency) == 0) {	//去掉top
						var transparencySet = parseInt(params.transparency);//去掉top

...
STAMP.Analysis = function(earth) {
	...
	var _showMeasureResult = function(result) {
		...
		var html = "<html><body style='background: " + bc + "; color: " + c +
			"; font-weight: bold; margin: 0; padding: 2px;'><p style='text-align:center;margin-top:30px;' >" +
			result + "</p></body></html>";
		...
		if(SYSTEMPARAMS.balloonAlpha == 0) {
			...
			htmlBal.SetBackgroundRGB(0x464750);
			htmlBal.SetScreenLocation(300 / 2 + 80 + 135, -20);
			...
			htmlBal.SetScreenLocation(250 / 2 + 80 + 135, 0);
		}
	...
	var showMoveHtml = function(anaLysisChk) {
		...
		earth.Event.OnDocumentReadyCompleted = function(guid) {
			...
			earth.analysisObj = parent.analysis;
			...
			earth.Tools = Tools;
			try {
				if(parent.getOperObject() && parent.getOperObject().$("#userDataTree")) {
					earth.tempUserdataTree = parent.getOperObject().$.fn.zTree.getZTreeObj("userDataTree");
				}
			} catch(e) {

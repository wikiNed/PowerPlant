...
STAMP.Userdata = function(earthObj) {
	...
	function updateTree(data) {
		...
		try {
			if(getOperObject() && getOperObject().$("#userDataTree") && getOperObject().$("#userDataTree").length > 0) {	//去掉top
				var tempzTree = getOperObject().$.fn.zTree.getZTreeObj("userDataTree");
				if(tempzTree == null) {
					return;
				}
	...
	var setRightDisable = function(isDisable) {
		try {
			if(getOperObject() && getOperObject().$("#userDataTree") && getOperObject().$("#userDataTree").length > 0) {	//去掉top
				getOperObject().setRightDisable(isDisable)
			}
		} catch(e) {
	...
	var _delObj = function() {
		...
		if(selObjNum <= 0 || selObjNum == "") {
			...
			if(r) {
				if(isUserdataTree) {	//去掉top
					var thisZTree = getOperObject().getUserdataTree();	//去掉top
				}
				var selectObjs = [];
	 ...
	 * @return {[type]}      [description]
	 */
	function removeObjFromEarth(nodeID) { //todo 根据GUID删除earth上的元素
		//清除earth上对应的要素
		earth.ShapeCreator.Clear();
		var rootxml = getUserdata(filename);

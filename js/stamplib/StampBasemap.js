...
function unloadEarth() {
	if(LayerManagement.htmlBalloon != null) {	//去掉top
		LayerManagement.htmlBalloon.DestroyObject();//去掉top
		LayerManagement.htmlBalloon = null;//去掉top
	}
	...
	if(dialogId == "mCamera") {
		if(earth.UserDocument) {
			for(var id in cameras) {	//去掉top
				if(cameras.hasOwnProperty(id)) {	//去掉top
					if(cameras[id] != "") {	//去掉top
						cameraLayer.DetachWithDeleteObject(cameras[id]);	//去掉top
					}
				}
			}
			earth.DetachObject(cameraLayer);	//去掉top
			cameras = [];	//去掉top
			cameraLayer = null;	//去掉top
		}
	}
	try {
		if(seearth) {

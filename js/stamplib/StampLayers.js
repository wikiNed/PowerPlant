...
var LayerManagement = {
	...
	getLayerTreeData: function(layer, isDialog) {
		if(!layer) {
			layer = LayerManagement.earth.LayerManager.LayerList;	//去掉top
		}
		var layerData = [];
		var childCount = layer.GetChildCount();
		for(var i = 0; i < childCount; i++) {
		...
		if(isShow) {
			...
			LayerManagement.earthArray[0].Event.OnGUISliderChanged = function(id) {
				...
				// } else {
					...
					// var thisSliderText = seHistorySliderMgr.data[thisId]['slider'].CurrentHistoryDateTimeTxt;	//去掉top
					// LayerManagement.setOtherSliderChaged(thisId, thisSliderText);	//去掉top
				// }
			}
		} else {
			if(LayerManagement.earthArray && LayerManagement.earthArray.length > 0) {
	...
	var setting = {
		...
		callback: {
			...
			onDblClick: function(event, treeId, node) { //双击图层
				LayerManagement.layerTreeDbClick(earthObj, node);	//去掉top
			},
			onCheck: function(event, treeId, node) { //点击checkbox事件
				LayerManagement.layerTreeCheck(earthObj, node);	//去掉top
			}
		}
	};
	var rootLayer = LayerManagement.getRootLayer(earthObj);

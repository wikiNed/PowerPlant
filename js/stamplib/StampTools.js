...
Stamp.Tools = {
	...
	ViewTranSetting: function(id) { //地形透明
		var flag = EarthMenu.isMenuClicked(id);//是否被选中
		if(flag) {
			setSlidersVisible(1);
	...
	showMoveHtmlBalloons: function(url, w, h) { //显示气泡
		...
		earth.Event.OnDocumentReadyCompleted = function(guid) {
			...
			earth.dataProcess = getDataProcessIndex();	//去掉top
...
function setSlidersVisible(flag) {
	...
	sliderMgr.init(Stamp.Tools.Earth, false, function(type) {
		for(var i in st) {
			if(st[i].type == type) {
				Tools.groupItemCancel(st[i].id);	//去掉top
				if(type == "transparency" ) {
					ViewTranSettingBtn = false;	//去掉top
					//BalloonHtml.removeItemStle(st[i].id);	//EarthMenu Modify
					EarthMenu.cancelMenuClick(st[i].id);

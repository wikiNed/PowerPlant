...
//弹出工具栏菜单
function showEarthTools() {	// EarthMenu Modify
...
var Tools = {
	...
	toolBarItemClickStyle: function(command) {
		//if($("#" + command).hasClass('selected') || (earthToolsDiv && earthToolsDiv.find("#" + command).attr("isChecked"))) {	//去掉top
		if($("#" + command).hasClass('selected') || (EarthMenu.isMenuClicked(command))) {
			$("#" + command).removeClass('selected');
...
var ThreeMenu = {
	itemClickStyle: function(id) {
		...
		if($.inArray(id, effctArr) >= 0) {
			/*
			if(earthToolsDiv && earthToolsDiv.find("#ViewTranSetting").attr("isChecked")) {	//去掉top

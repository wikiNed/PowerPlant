...
var userdata = parent.userdata; //用户数据
...
$("#divBeginExplosion").live("click", function(){
	...
	if(nodes && nodes.length > 0) {
		...
		var userDataObj = userdata.getUserdataByGuid(node.id);
		if(userDataObj){
			var explosionFlag = parent.LayerManagement.earth.Factory.CreateGuid();//唯一标识
...
$(function() {
	...
	window.onunload = function() {
		if(parent.LayerManagement.earth.ShapeCreator) {
			parent.LayerManagement.earth.ShapeCreator.Clear();
		}
		parent.isUserdataTree = false;
	}
});